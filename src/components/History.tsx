...
const History = ({ conversations, activeConversationId, setActiveConversationId, setConversations, isCollapsed, toggleCollapse }: HistoryProps) => {
  ...
  return (
    <div className={`relative bg-gray-800 flex flex-col transition-all duration-300 ease-in-out ${isCollapsed ? 'w-16' : 'w-64'}`}>
        <div className={`p-4 flex items-center ${isCollapsed ? 'justify-center' : 'justify-between'}`}>
            {!isCollapsed && <h2 className="text-lg font-bold whitespace-nowrap">历史对话</h2>}
            <button onClick={toggleCollapse} className="p-1 hover:bg-gray-700 rounded-full">
              {isCollapsed ? (
      ...
      <div className={`flex-1 overflow-y-auto overflow-x-hidden transition-opacity duration-200 ${isCollapsed ? 'opacity-0 pointer-events-none' : 'opacity-100'}`}>
        {conversations.map(conversation => (
          <div 
            key={conversation.id} 
            className={`p-2 mx-2 my-1 rounded-md cursor-pointer flex justify-between items-center ${activeConversationId === conversation.id ? 'bg-blue-600' : 'hover:bg-gray-700'}`}
          >
